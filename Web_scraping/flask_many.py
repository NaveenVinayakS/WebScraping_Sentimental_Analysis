    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if reviews.count() > 0:  # if there is a collection with searched keyword and it has records in it
                ...
                n = 4
                bigboxes = bigboxes[:len(bigboxes) - n]
                print("Length of Boxes", len(bigboxes))
                for box in bigboxes:
                    productLink = "https://www.flipkart.com" + box.div.div.div.a['href']  # extracting the actual product link
                    prodRes = requests.get(productLink)  # getting the product page from server
                    prod_html = bs(prodRes.text, "html.parser")  # parsing the product page as HTML
                    commentboxes = prod_html.find_all('div', {'class': "_16PBlm"})  # finding the HTML section containing the customer c
                    table = db[
                    ...
                    print("Length of comment boxes :-", len(commentboxes))
